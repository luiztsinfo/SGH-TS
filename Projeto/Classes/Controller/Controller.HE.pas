...
uses Lca.Orm.Comp.FireDac, Model.HE, Firedac.Comp.Client, Conexao, Data.DB,
  ...
  Model.Prescricao_Enfermagem_HE, Model.PrescricaoEnfermagem;
...
type
  ...
  private
    ...
    procedure SetDsPrescricao(const Value: TDataSource);
    procedure SetDsPrescricao_HE(const Value: TDataSource);
    procedure SetQryPrescricoes_HE(const Value: TFDQuery);
    procedure SetRegistrosPrescricoes(const Value: TDataSet);
  ...
  public
    ...
    FPrescricao_Enfermagem_HE: TPrescricao_enfermagem_HE;
    ...
    property DsPrescricao_HE: TDataSource read FDsPrescricoes_HE write SetDsPrescricao_HE;
...
begin
  ...
  FPrescricaoEnfermagem := TPrescricoes_enfermagem.Create;
  FPrescricao_Enfermagem_HE := TPrescricao_enfermagem_HE.Create;
  ...
  FQryPrescricoes_HE.Connection := FConexao.FdCon;
  ...
  FDsPrescricoes_HE.DataSet := FQryPrescricoes_HE;
end;
...
begin
  ...
  FPrescricaoEnfermagem.Free;
  FPrescricao_Enfermagem_HE.Free;
  ...
  FRegistrosPrescricoes.Free;
  FQryPrescricoes_HE.Free;
...
function TControllerHE.GravaPrescricoes_HE: boolean;
begin
  FDao.Inserir(FPrescricao_Enfermagem_HE);
end;
...
procedure TControllerHE.MostraPrescricoes;
begin
  try
    QryPrescricoes_HE.Close;
    QryPrescricoes_HE.SQL.Clear;
    QryPrescricoes_HE.SQL.Add('SELECT phe.id, phe.id_he, phe.id_prescricao_enfermagem,');
    QryPrescricoes_HE.SQL.Add('p.descricao As Prescricao');
    QryPrescricoes_HE.SQL.Add('FROM prescricao_enfermagem_he phe');
    QryPrescricoes_HE.SQL.Add('INNER JOIN prescricoes_enfermagem p');
    QryPrescricoes_HE.SQL.Add('ON p.id = phe.id_prescricao_enfermagem');
    QryPrescricoes_HE.SQL.Add('WHERE phe.id_he = :he');
    QryPrescricoes_HE.ParamByName('he').AsInteger := FModel.ID;
    QryPrescricoes_HE.Open();
  except
    MessageDlg('Erro ao mostrar prescrições!',mtError,[mbOK],0);
  end;
end;
...
function TControllerHE.RemoverPrescricao_HE: boolean;
begin
  try
    Result := false;
    FPrescricao_Enfermagem_HE.Id := QryPrescricoes_HE.FieldByName('id').AsInteger;
    FDao.Excluir(FPrescricao_Enfermagem_HE);
  finally
    Result := true;
  end;
end;
...
function TControllerHE.RetornaPrescricaoPorDescricao: string;
begin
  FRegistrosPrescricoes := FDao.ConsultaTab(FPrescricaoEnfermagem,['id','descricao'],['descricao','situacao'],['descricao'],comLike);

  FPrescricaoEnfermagem.Id := FRegistrosPrescricoes.FieldByName('id').AsInteger;
  FPrescricaoEnfermagem.Descricao := FRegistrosPrescricoes.FieldByName('descricao').AsString;

  FDsPrescricao.DataSet := FRegistrosPrescricoes;

  Result := FRegistrosPrescricoes.FieldByName('descricao').AsString;
end;
...
procedure TControllerHE.SetDsPrescricao(const Value: TDataSource);
begin
  FDsPrescricao := Value;
end;

procedure TControllerHE.SetDsPrescricao_HE(const Value: TDataSource);
begin
  FDsPrescricoes_HE := Value;
end;
...
procedure TControllerHE.SetQryPrescricoes_HE(const Value: TFDQuery);
begin
  FQryPrescricoes_HE := Value;
end;
...
procedure TControllerHE.SetRegistrosPrescricoes(const Value: TDataSet);
begin
  FRegistrosPrescricoes := Value;
end;
